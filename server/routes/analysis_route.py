...
from .analysis_route_utils import (
    fetch_csv, create_bar_chart, create_plot_chart, filter_sensors, filter_date_range, compute_summary_stats
)
...
async def daily_humidity_graph(
    ...
    if filtered_df.empty:
        return JSONResponse(content={"error": f"No data available from {start_date} to {end_date}"}, status_code=404)

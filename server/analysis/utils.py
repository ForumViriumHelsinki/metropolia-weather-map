...
def get_makela_sensors():
    SENSORS, SENSOR_SUN, SENSOR_SHADE = get_sensors()  # Get sensor data
    return SENSORS, SENSOR_SUN, SENSOR_SHADE  # Return sensor lists, first all, second sunlight, third shade
...
def separate_sensors(sensor_df):
    filtered = {
        ...
        if sensor_id in get_makela_sensors()[0]
    }
    return filtered
